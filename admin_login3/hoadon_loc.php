...
echo "
    <div class='filter-container'>
        ...
        <form method='GET' class='filter-form'>
            ...
            <a href='index.php?page=hoadon_loc' class='btn-filter'>Xóa lọc</a>
        </form>
...
<div class='filter-title'>Lọc đơn hàng theo tình trạng đơn hàng</div>
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        echo "<tr>
                ...
                <button type='button' class='btn-filter' onclick=\"showDetails('{$row['mahd']}')\">Xem chi tiết</button></td>
              </tr>";
    }
    echo "</table>";
...
function renderTable(data) {
    ...
    if (data.length === 0) {
        ...
        data.forEach(row => {
            html += `
                <tr>
                    <td data-label="Họ và tên">${row.hoten || row.username}</td>
                    <td data-label="Số điện thoại">${row.SDT || ''}</td>
                    <td data-label="Mã hóa đơn">${row.mahd}</td>
                    <td data-label="Ngày đặt">${row.ngaydat}</td>
                    <td data-label="Thanh toán">${row.phuongthucthanhtoan}</td>
